...
int main(int argc, char* argv[]) {
 ...
 # ifdef DEBUG
      Read_matrix("Ingresar matriz", A, m, n);
      Print_matrix("Se lee", A, m, n);
      Read_vector("Ingresar vector", x, n);
      Print_vector("Se lee", x, n);
 # else
      Gen_matrix(A, m, n);
...
void Usage (char* prog_name) {
   fprintf(stderr, "uso: %s <numero de hilos> <m> <n>\n", prog_name);
   exit(0);
}
...
// Multiplicacion matriz - vector del libro
void Omp_mat_vect(double A[], double x[], double y[],
      int m, int n, int thread_count) {
   int i, j;
